...
#### Viruses: 
These are files that can be very dangerous to a computer due to its ability to copy itself and corrupt or destroy data within a computer. It does this by infecting files within the system, when a file is infected it can be corrupted or destroyed entirely, this means that depending from where the viruses originates, it can be extremely dangerous for systems as primary or high priority files can be corrupted or destroyed.  These can be detected with anti malware software, or by having methods to stop access into the data files entirely, this can mean a firewall or a simple auto scanning software, however because it conceals itself within files a full scan is usually required in order to find and remove the folders.

#### Trojan Horse: 
The name comes from the anchient device known as the Trojan horse that was responsible for the fall of troy, it is a form of social engineering where a piece of software is hidden within an email or other form of message and is built to bait people into downloading and/or running a piece of software. The package that the horse can contain can be of any kind, so it could not be malicous at all, however it is most common to find malicous malware such as viruses hidden within the trojan to cause harm to a PC.
...
#### Worm: 
This is the most common type of malware, its purpose is to clone itself and spread through the network the system is on, this could be the LAN of the system or in a company it could be the it's intranet that it uses. Worms main focus and functions is to cause a disrupt within a system, whether it be the bandwidth that it uses or the speed of the system itself, however some worms are used to attack pieces of security products like firewalls and can cause slow functioning security measures, this itself is very dangerous and can be used to open a securities security protocol. 
